...
procedural skills usually use Opus at low effort, and routine skills use Haiku. The project-local
`.claude/skills/plans-revision/SKILL.md` is the lone `model: sonnet` + `effort: high` outlier.
Round-1 pricing and quality evidence backs Opus at lower effort over Sonnet.
...
- Bad: Existing outliers must be re-graded; Round 3 updates `plans-revision` /
  `review-implementation-plans` to Opus at low effort.
