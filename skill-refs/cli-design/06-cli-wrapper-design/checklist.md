...
- [ ] Conflicting flags (wrapper vs child) are documented; the wrapper's grammar wins for _its_ own flags only.
...
- [ ] Wrapper-specific exit codes (e.g. for usage errors _in the wrapper itself_) are distinct from child codes and follow BSD sysexits.
