...
Values in angle brackets are placeholders. Copy-paste starting points per language are in the cog
nix template domain (`cog skill-refs path templates/nix/<type>`).
...
attribute into an exported env var, so this is a clean one-liner (already baked into the python
template, `templates/nix/python/flake.nix`):
