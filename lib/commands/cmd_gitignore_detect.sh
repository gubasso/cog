...
__cog_gitignore_detect_usage() {
  cog::fn::ui_data "Usage: cog gitignore-detect [--project-root <dir>] [--template-root <dir>] [--type <type>] (<out.json>|--json)"
}
  ...
  while (($# > 0)); do
    case "$1" in
      ...
      --project-root)
        ...
        ;;
      --template-root)
        [[ $# -ge 2 ]] || cog::fn::error_raise "MissingArgument" "missing template root" "option: --template-root" "" "run 'cog gitignore-detect --help'"
        template_root="$2"
        shift 2
        ;;
      --type)
