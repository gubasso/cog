...
2026-08-14, at review: `--access write` is the whole write declaration for `claude-runner`, with no confirmation flag beside it. Raised by the review loop, which found the orchestration contract asserting that write "is admitted only by `--mode danger`" while the new runner admitted it on `--access` alone. Resolved toward the better CLI rather than the mechanical mirror: codex checks `--mode` and `--access` for coherence because `--mode` carries the sandbox strategy, information `--access` does not; Claude exposes no sandbox to choose, so a second flag would ask for a fact the caller had already typed. The contract was rewritten to state the rule that is actually true of both runners.

2026-08-14, at implementation: three shaping decisions the slice left open. The launch target is `claude-session-rs`, the wrapper the measured mapping was taken against — it is not installed on the development host, so `gate` and `run-exec` fail closed there and every test runs against a fake shim; no lane was proven against a live Claude job. Read-only access is spelled `--permission-mode dontAsk --disallowedTools "Edit,Write,NotebookEdit"` rather than the mapping's bare `--permission-mode`, because `claude` has no read-only mode and `plan` changes what the agent does rather than only what it may touch. The whole remainder was implemented, including the cut-first `codex-runner` retrofit. One correction fell out of the work: `cog::fn::executor::validate_engine` echoes its argument, so the new `--prepare-engine` validation had to be silenced at both call sites.
