...
__cog_refactor_scan_drift_build_json() {
  ...
  if [[ -n $expected ]]; then
    ...
    expected_json=null; matches_json=null
  fi
...
cog::cmd::refactor_scan_drift() {
  ...
  while (($# > 0)); do
    case "$1" in
      -h | --help) __cog_refactor_scan_drift_usage; return 0 ;;
      --scan) [[ $# -ge 2 ]] || cog::fn::error_raise "MissingArgument" "missing scan dir" "option: --scan" "" "run 'cog refactor-scan-drift --help'"; scan="$2"; shift 2 ;;
      --expected) [[ $# -ge 2 ]] || cog::fn::error_raise "MissingArgument" "missing expected fingerprint" "option: --expected" "" "run 'cog refactor-scan-drift --help'"; expected="$2"; shift 2 ;;
      --json) [[ -z $mode ]] || cog::fn::error_raise "InvalidInput" "duplicate refactor-scan-drift output mode" "" "" "choose either --json or an output path"; mode=json; shift ;;
      -*) cog::fn::error_raise "InvalidInput" "unknown refactor-scan-drift option" "option: $1" "" "run 'cog refactor-scan-drift --help'" ;;
      *) [[ -z $mode && -z $out ]] || cog::fn::error_raise "TooManyArguments" "too many refactor-scan-drift output paths" "argument: $1" "" "run 'cog refactor-scan-drift --help'"; out="$1"; mode=file; shift ;;
    esac
  done
