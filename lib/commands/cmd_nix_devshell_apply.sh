...
__cog_nix_devshell_apply_usage() {
  cog::fn::ui_data "Usage: cog nix-devshell-apply --type <type> [--project-root <dir>] [--template-root <dir>] [--flake-conflict overwrite|skip|abort] [--envrc-conflict overwrite|skip|abort] [--companion-conflict overwrite|skip|abort] (<out.json>|--json)"
}
  ...
  while (($# > 0)); do
    case "$1" in
      ...
      --project-root)
        ...
        ;;
      --template-root)
        [[ $# -ge 2 ]] || cog::fn::error_raise "MissingArgument" "missing template root" "option: --template-root" "" "run 'cog nix-devshell-apply --help'"
        template_root="$2"
        shift 2
        ;;
      --flake-conflict)
