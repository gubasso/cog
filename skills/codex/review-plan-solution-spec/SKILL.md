...
Review a target solution bundle before implementation. Keep the review producer-blind: name only the solution bundle, public capability bundle, denylist path, and shared contracts supplied in the prompt.
...
Evaluate target architecture fit, new interface design, implementation sequencing, requirement coverage, Layer-B acceptance tests, documentation updates, target-stack scoping, and behavioral sections that must remain source-agnostic.
...
Emit a structured plan-review verdict with `APPROVED`, `MODIFIED`, `REMOVED`, and `ADDED` sections. End with `pass` only when the solution bundle is implementable, trace-complete, and clean under the scoped leakage gate. Otherwise end with `revise` and concrete required changes.
