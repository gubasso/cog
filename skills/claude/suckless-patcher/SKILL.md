...
to the Codex `suckless-patcher` skill.
...
2. Research the patch URL with `web_fetch`. Extract description, latest available version,
   dependencies, and compatible suckless versions. If the user's patch appears stale, explain the
   difference and ask whether to proceed or download the latest.
