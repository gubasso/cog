...
Single source of truth for the Phase-0 plan-mode gate. Every Claude `executor-*`/`runner-*` orchestrator carries a short in-body imperative that points here; this file owns the full protocol. The gate lives on the orchestrator layer — the caller a user launches gates once at entry, then delegates to gate-free plan/review workers. Plan-mode detection stays probabilistic in skill prose.
