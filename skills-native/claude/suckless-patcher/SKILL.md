...
description: >
  ...
  and conflict artifact listing to the cog CLI while preserving patch research
...
```bash
RUN_DIR="$(cog rundir suckless-patcher | sed -n 's/^RUN_DIR=//p')"
...
2. Research the patch URL with `web_fetch`. Extract description, latest available version, dependencies, and compatible suckless versions. If the user's patch appears stale, explain the difference and ask whether to proceed or download the latest.
...
5. Resolve conflicts in prose and with targeted edits. For each rejected hunk, identify the intended behavior, apply the intent manually, and explain the edit to the user.
