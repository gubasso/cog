...
runs `/review-plan-claude` in orchestrator mode with three absolute paths and writes the reviewed
...
read the run-dir artifacts directly. `review-plan-claude` owns the `cog plan-review orchestrator`
...
- `prompt` (substitute the literal value of `$RUN_DIR` before sending):
  ...
  ```text
  Read the skill file at $HOME/.claude/skills/review-plan-claude/SKILL.md
  and run /review-plan-claude in orchestrator mode. Its three absolute path
  arguments are:
