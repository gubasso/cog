...
: 'desc: Verify a review-implementation-plans run against a before/after scan.'

__cog_review_implementation_plans_verify_scan_check='
  (.ok == true) and
...
__cog_review_implementation_plans_verify_self_check='
  (.ok == true) and
...
__cog_review_implementation_plans_verify_usage() {
  cog::fn::ui_data "Usage: cog review-implementation-plans-verify --before <scan.json> --after <scan.json> [--allow-noop] (<out.json>|--json)"
}

__cog_review_implementation_plans_verify_read_scan() {
  local path="${1:-}"
  [[ -n $path ]] || cog::fn::error_raise_with_exit "$EX_USAGE" "MissingArgument" \
    "missing scan path" "function: review-implementation-plans-verify" "" ""
  [[ -f $path ]] || cog::fn::error_raise "InputNotFound" \
    "scan file not found" "path: ${path}" "" "check the scan path"
  jq -e "$__cog_review_implementation_plans_verify_scan_check" "$path" >/dev/null || cog::fn::error_raise "InvalidInput" \
    "malformed review-implementation-plans scan" "path: ${path}" "" "rerun review-implementation-plans-scan"
  cat -- "$path"
}

__cog_review_implementation_plans_verify_validate_after_queues() {
...
__cog_review_implementation_plans_verify_history_preserved() {
...
__cog_review_implementation_plans_verify_build_json() {
  local before_file="$1" after_file="$2"
  local before_json after_json changed completed_history_preserved=true

  before_json="$(__cog_review_implementation_plans_verify_read_scan "$before_file")"
  after_json="$(__cog_review_implementation_plans_verify_read_scan "$after_file")"
  __cog_review_implementation_plans_verify_validate_after_queues "$after_json"

  if ! __cog_review_implementation_plans_verify_history_preserved "$before_json" "$after_json"; then
...
cog::cmd::review_implementation_plans_verify() {
    ...
    case "$1" in
      ...
      --before)
        [[ $# -ge 2 && -z $before ]] || cog::fn::error_raise_with_exit "$EX_USAGE" "MissingArgument" \
          "missing before scan" "option: --before" "" "run 'cog review-implementation-plans-verify --help'"
        before="$2"
        shift 2
        ;;
      --after)
        [[ $# -ge 2 && -z $after ]] || cog::fn::error_raise_with_exit "$EX_USAGE" "MissingArgument" \
          "missing after scan" "option: --after" "" "run 'cog review-implementation-plans-verify --help'"
        after="$2"
        shift 2
        ;;
      --allow-noop)
      ...
      --json)
        [[ -z $mode ]] || cog::fn::error_raise_with_exit "$EX_USAGE" "InvalidInput" \
          "duplicate review-implementation-plans-verify output mode" "" "" "choose either --json or an output path"
        mode=json
        shift
        ;;
      -*)
        cog::fn::error_raise_with_exit "$EX_USAGE" "InvalidInput" \
          "unknown review-implementation-plans-verify option" "option: $1" "" "run 'cog review-implementation-plans-verify --help'"
        ;;
      *)
        [[ -z $mode && -z $out ]] || cog::fn::error_raise_with_exit "$EX_USAGE" "TooManyArguments" \
          "too many review-implementation-plans-verify output paths" "argument: $1" "" \
          "run 'cog review-implementation-plans-verify --help'"
        out="$1"
        mode="file"
        shift
        ;;
  ...
  [[ -n $before && -n $after ]] || cog::fn::error_raise_with_exit "$EX_USAGE" "MissingArgument" \
    "missing review-implementation-plans-verify scan" \
    "usage: cog review-implementation-plans-verify --before <scan.json> --after <scan.json> [--allow-noop] (<out.json>|--json)" "" \
    "run 'cog review-implementation-plans-verify --help'"
  [[ -n $mode || ${COG_UI_JSON:-false} == true ]] || cog::fn::error_raise_with_exit "$EX_USAGE" "MissingArgument" \
    "missing review-implementation-plans-verify output mode" "usage: cog review-implementation-plans-verify ... (<out.json>|--json)" "" \
    "run 'cog review-implementation-plans-verify --help'"
  [[ -n $mode ]] || mode=json
  json="$(__cog_review_implementation_plans_verify_build_json "$before" "$after")"
  if [[ $mode == json || ${COG_UI_JSON:-false} == true ]]; then
    cog::fn::json_emit "$__cog_review_implementation_plans_verify_self_check" "$json"
  else
    cog::fn::json_write_fragment "$out" "$__cog_review_implementation_plans_verify_self_check" "$json"
  fi
}
