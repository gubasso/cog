...
`plan-oneshot` **inline** via the `Skill` tool with the orientation, capturing its lean plan-doc to
`DRAFT_PATH`:
...
- Use `Skill` to chain `plan-oneshot` / `review-plan-complexity` / `plan-split` inline (same context).
