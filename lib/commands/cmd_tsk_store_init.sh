...
cog::cmd::tsk_store_init() {
  ...
  while (($# > 0)); do
    case "$1" in
      -h | --help) __cog_tsk_store_init_usage; return 0 ;;
      --json) [[ -z $mode ]] || cog::fn::error_raise "InvalidInput" "duplicate tsk-store-init output mode" "" "" "choose either --json or an output path"; mode=json; shift ;;
      -*) cog::fn::error_raise "InvalidInput" "unknown tsk-store-init option" "option: $1" "" "run 'cog tsk-store-init --help'" ;;
      *) [[ -z $mode && -z $out ]] || cog::fn::error_raise "TooManyArguments" "too many tsk-store-init output paths" "argument: $1" "" "run 'cog tsk-store-init --help'"; out="$1"; mode=file; shift ;;
    esac
  done
