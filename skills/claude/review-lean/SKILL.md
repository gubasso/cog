...
Parse `[--scope <glob>] [--severity <min>] [--format markdown|json] [--comment]` from `$ARGUMENTS`.
...
Force JSON output, skip prompts, run Phases 0-2, write the JSON document to `<output-path>`, and
reply:
