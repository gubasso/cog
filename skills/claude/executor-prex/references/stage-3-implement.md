...
3. The full content of `$RUN_DIR/stage2-reviewed-plan.md` (inlined, not referenced). When the plan is
...
6. The implementation instructions: implement phases in order, avoid silent deviations, report files
   ...
   header rule that stage 4 parses.

Keep the prompt-file rule: write the complete prompt to a file inside `RUN_DIR` first (e.g.
`$RUN_DIR/stage3-prompt.md`), then pass it to `cog codex-runner run-exec`. Do not inline multi-line
...
error), inspect `$RUN_DIR/stage3-stderr.log`, report the failure, release the lock, and ask the user
whether to retry or abort. Do not retry automatically.

Extract the implementation thread ID (stage 4 may reference it):
