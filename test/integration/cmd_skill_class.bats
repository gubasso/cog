...
@test "cog skill-class check fails a plan skill carrying a plan-mode gate" {
  write_skill "${BATS_TEST_TMPDIR}/skills/claude/plan-bad" plan-bad '<!-- cog-plan-mode-gate -->'
  run cog skill-class check --skill "${BATS_TEST_TMPDIR}/skills/claude/plan-bad/SKILL.md" --json
  assert_failure
  [[ "$(jq -r '.forbidden_present | join(",")' <<<"$output")" == *"plan-mode-gate"* ]]
}

@test "cog skill-class check fails an executor skill missing the plan-mode gate" {
  write_skill "${BATS_TEST_TMPDIR}/skills/claude/executor-bad" executor-bad 'no gate here'
  run cog skill-class check --skill "${BATS_TEST_TMPDIR}/skills/claude/executor-bad/SKILL.md" --json
  assert_failure
  [[ "$(jq -r '.missing | join(",")' <<<"$output")" == *"plan-mode-gate"* ]]
}
