...
cog::fn::skill_class::governed_classes() {
  printf '%s\n' plan review review-plan executor runner
}

cog::fn::skill_class::is_governed_class() {
  case "${1:-}" in
    plan | review | review-plan | executor | runner) return 0 ;;
    *) return 1 ;;
  esac
}
...
cog::fn::skill_class::show_json() {
  ...
  cog::fn::skill_class::is_governed_class "$class" || cog::fn::error_raise "InvalidInput" \
    "unknown skill class" "class: ${class}" "expected plan|review|review-plan|executor|runner" \
