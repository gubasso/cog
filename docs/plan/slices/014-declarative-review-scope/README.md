...
2026-08-22, at review: the merge-commit rabbit hole above is wrong, and slice 015 corrects it. `git show --numstat` does not report nothing for a merge — it reports first-parent numstat. What reports nothing is `git show --name-only`, which this slice used for the file list while using `--numstat` for the stats. The two disagreed: a merge selector produced an empty `commit_files` beside a `diff_stats.commits` that named a file, so the reviewer never opened a file the budget was charged for. Recorded here rather than edited above, because the entry is part of what this slice actually believed.
