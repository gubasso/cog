...
Bare `ADR-NNNN` citations must name a number in cog's live domain, currently `0001`..`0027`:
...
rg -n --no-heading -o 'ADR-[0-9]{4}' \
  ...
  | awk -F'ADR-' '{n=substr($2,1,4)+0; if (n<1 || n>27) print "out-of-domain ADR reference: " $0}'
```
