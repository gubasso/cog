...
`$ARGUMENTS` is `--run-dir <dir>` followed by the original executor input, verbatim. The input may be free text, one or more plan file paths, directory references, or any mix. The verdict is written under `<run-dir>`.
