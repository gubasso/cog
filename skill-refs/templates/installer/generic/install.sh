...
# Manifest-driven installer (language-agnostic starting point).
#
# It copies a project-owned payload into an XDG/PREFIX destination, links any
# executables onto PATH, records every installed path in a manifest, and
# stale-prunes files a previous install shipped but this one no longer does.
# uninstall.sh reverses it from the same manifest.
#
# This is the generic scaffold: fill in the PROJECT CONFIGURATION block with
# your project's real payload. The reusable machinery below rarely changes.
#
# Environment:
...
#
#   project_name   drives the install roots ($prefix/lib/<name>, etc.).
#   copy_trees     "<src-dir>|<dest-dir>"          recursive copies
#   copy_files     "<src-file>|<dest-path>|<mode>" individual files
#   bin_links      "<link-target>|<link-path>"     PATH symlinks
#   owned_clear    dirs/files wiped before re-copy (project-owned only!)
#   manifest_roots roots every installed path must fall under
# ============================================================================
...
copy_trees=()
copy_files=(
  "bin/$project_name|$app_root/bin/$project_name|0755"
)
bin_links=(
