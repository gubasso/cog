---
name: review-plan-solution-spec
description: >
  Review a target solution bundle against the sanitized capability contract,
  requirement traceability, target references, and scoped behavioral leakage
  policy.
argument-hint: "<validated-brief-path>"
disable-model-invocation: true
allowed-tools: Bash Read Grep Glob
---

<!-- trigger-tests: "review-plan-solution-spec", "review solution spec", "audit target solution traceability" -->
<!-- cog-skill: plan-emitter -->

# Review Plan Solution Spec

Review a target solution bundle before implementation. Keep the review producer-blind: name only the
solution bundle, public capability bundle, denylist path, and shared contracts supplied in the brief.

## Inputs

`$ARGUMENTS` is a validated context brief path. The brief supplies:

- solution bundle paths;
- public capability bundle paths;
- private denylist path for scoped behavioral-section scans;
- solution contract path;
- capability contract path;
- leakage policy path;
- user reference-doc paths used by the target design.

If the brief is missing or invalid, stop and ask the caller for a validated brief path.

## Review Axes

Evaluate:

- solution bundle structure against the solution contract;
- target architecture fit to the user reference docs;
- new interface design clarity;
- implementation plan completeness and sequencing;
- Layer-A requirement coverage in `requirement-trace.yaml`;
- Layer-B acceptance tests against the new interface;
- documentation and decision updates;
- target-stack terms scoped to target-design sections;
- behavioral sections free of source-specific leakage.

## Scoped Leakage Gate

The solution bundle intentionally contains target-stack terms. Scan only behavioral sections that
should remain source-agnostic:

```bash
cog spec-leakage-scan <behavioral-section-files> --source-denylist <denylist-path> --json
```

Any behavioral-section finding is a blocking `revise` verdict. Do not treat target architecture prose
as leakage merely because it names the chosen target stack.

## Output

Emit a structured plan-review verdict using the shared vocabulary:

- `APPROVED` for complete, correct target design material;
- `MODIFIED` for material that needs revision;
- `REMOVED` for leaked, unsupported, or misplaced material;
- `ADDED` for missing traceability, tests, design details, or docs.

End with `pass` only when the solution bundle is implementable, trace-complete, and clean under the
scoped leakage gate. Otherwise end with `revise` and concrete required changes.
