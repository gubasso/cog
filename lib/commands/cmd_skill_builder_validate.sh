...
cog::cmd::skill_builder_validate() {
  ...
  while (($# > 0)); do
    case "$1" in
      -h | --help)
        ...
        ;;
      --name)
        [[ $# -ge 2 && -z $name ]] || cog::fn::error_raise "MissingArgument" "missing skill name" "option: --name" "" "run 'cog skill-builder-validate --help'"
        name="$2"
        shift 2
        ;;
      --scope)
        [[ $# -ge 2 ]] || cog::fn::error_raise "MissingArgument" "missing skill scope" "option: --scope" "" "run 'cog skill-builder-validate --help'"
        scope="$2"
        shift 2
        ;;
      --project-root)
        [[ $# -ge 2 ]] || cog::fn::error_raise "MissingArgument" "missing project root" "option: --project-root" "" "run 'cog skill-builder-validate --help'"
        project_root="$2"
        shift 2
        ;;
      --home)
        [[ $# -ge 2 ]] || cog::fn::error_raise "MissingArgument" "missing home dir" "option: --home" "" "run 'cog skill-builder-validate --help'"
        home_dir="$2"
        shift 2
        ;;
      --run-dir)
        [[ $# -ge 2 ]] || cog::fn::error_raise "MissingArgument" "missing run dir" "option: --run-dir" "" "run 'cog skill-builder-validate --help'"
        run_dir="$2"
        shift 2
        ;;
      --draft)
        [[ $# -ge 2 && -z $draft_file ]] || cog::fn::error_raise "MissingArgument" "missing draft file" "option: --draft" "" "run 'cog skill-builder-validate --help'"
        draft_file="$2"
        shift 2
        ;;
      --json)
        [[ -z $mode ]] || cog::fn::error_raise "InvalidInput" "duplicate skill-builder-validate output mode" "" "" "choose either --json or an output path"
        mode=json
        shift
        ;;
      -*) cog::fn::error_raise "InvalidInput" "unknown skill-builder-validate option" "option: $1" "" "run 'cog skill-builder-validate --help'" ;;
      *)
        [[ -z $mode && -z $out ]] || cog::fn::error_raise "TooManyArguments" "too many skill-builder-validate output paths" "argument: $1" "" "run 'cog skill-builder-validate --help'"
        out="$1"
        mode="file"
        shift
        ;;
  ...
  if [[ -n $draft_file && -z $name ]]; then
    ...
    check="$__cog_skill_builder_validate_self_check"
  else
