...
1. **Registry pin** — the skill name appears in a per-tier `skills` list in
   ...
   single escape hatch: the known exceptions live here (`executor-prex` and `review-oneshot` ride
   high; the codex delegation launchers, `review-findings`, and `review-queue-rounds` ride low).
...
- `model-effort-tier`: a governed Claude skill's `model:`/`effort:` frontmatter resolves to a tier
  ...
  default (HIGH); the known exceptions (`executor-prex` and `review-oneshot` → high, the codex
  delegation launchers, `review-findings`, and `review-queue-rounds` → low) are registry pins, the single escape
