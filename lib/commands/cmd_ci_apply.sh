...
__cog_ci_apply_usage() {
  cog::fn::ui_data "Usage: cog ci-apply --target github|gitlab [--project-root <dir>] [--template-root <dir>] [--conflict overwrite|skip|abort] (<out.json>|--json)"
}
...
__cog_ci_apply_build_json() {
  local target="$1" project_root="$2" template_root="$3" conflict="$4"
  local ok=true reason="" template_dir="$template_root/$target"
  ...
  if [[ $ok == true ]]; then
    enum_status=0
    __cog_ci_apply_enumerate_operations "$template_dir" "$project_root" || enum_status=$?
...
cog::cmd::ci_apply() {
  local target="" project_root template_root conflict=abort mode="" out="" json
  project_root="$(pwd -P)"
  ...
  [[ -n $mode ]] || mode=json
  json="$(__cog_ci_apply_build_json "$target" "$project_root" "$template_root" "$conflict")"
