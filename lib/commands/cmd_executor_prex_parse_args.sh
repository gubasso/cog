...
__cog_executor_prex_parse_args_self_check='(.mode|type=="string") and (.tsk_impl|type=="number") and (.tsk_id|type=="string") and (.task_file|type=="string")'
    ...
    case "$1" in
      ...
      -*)
        cog::fn::error_raise_with_exit 2 "InvalidInput" \
          "unknown executor-prex flag" "option: $1" "" "expected -a, --auto, -ar, --auto-review, -t, or --tsk-impl"
        ;;
      *)
        break
        ;;
...
__cog_executor_prex_parse_args_build_json() {
  ...
  __cog_executor_prex_parse_args_parse "$raw" mode tsk_impl tsk_id task
