...
Skills that spawn Codex, delegate to agents, or otherwise orchestrate nested work must read and follow [Orchestration contract](./orchestration-contract.md). Choose Skill-inline composition or Agent-delegate isolation deliberately. Include the env-preflight requirement where a workflow depends on foreground execution. Never reintroduce the removed foreground hook; runtime no-backgrounding is an env-first contract asserted by `cog`, not a `PreToolUse` hook.
...
This taxonomy is related accepted skill governance alongside [ADR-0036](../decisions/ADR-0036-explicit-model-and-effort-at-every-delegation.md) (explicit model/effort) and [ADR-0017](../decisions/ADR-0017-skill-authoring-and-lint.md) (plan-mode gate); those ADRs are referenced here, not changed.
...
Per [ADR-0036](../decisions/ADR-0036-explicit-model-and-effort-at-every-delegation.md), model and effort are stated directly, never derived through a tier ladder, a power-grade cell, or a registry:

- **Every coding-agent launch a skill ships carries a literal `--effort <value>` on the command itself** — `cog codex-runner run-exec|run-resume` and `cog claude-runner run-exec` alike. The value is the provider's native vocabulary (`minimal|low|medium|high|xhigh` for Codex; `low|medium|high|xhigh|max` for Claude, with `none` meaning the flag is omitted).
- **`--model` appears only when a non-default model is required.** An omitted `--model` — and, outside launch commands, an omitted `--effort` — means one thing: the harness default is accepted, deliberately.
- **Frontmatter `model:`/`effort:` are free explicit values** chosen for what the skill itself does. Absent both, the skill rides the session default. Sonnet is never selected.
- **Review cadence:** a review skill's first (cold) round runs at `high` effort on the default model; subsequent (warm) rounds run at `medium` effort.

Prose never routes model or effort through the retired indirection vocabulary — "power-grade", the tier ladder ("the HIGH tier"), "cells", or `model@effort` slugs. A deliberate exception (e.g. a historical note) is recorded with an inline `<!-- cog-skill-lint: allow-model-effort-indirection <reason> -->` on the preceding line.

`cog skill-lint` enforces this with two rules: `model-effort-indirection` (runtime `SKILL.md` prose, skipping frontmatter and fenced code blocks) and `agent-launch-explicit-effort` (fenced launch commands in runtime `SKILL.md` bodies and their `references/*.md` companions).
...
Enforcement is the `input-fidelity` lint rule, keyed off a curated runtime-aware delegator set in `lib/commands/cmd_skill_lint.sh` (which includes `context-builder` and `review-loop`). The marker name is retained for stability; its meaning is fidelity to intent and substance, not verbatim copying. The marker asserts the contract structurally; the best-constructed standard remains prose judgment in the skill body.
...
- `model-effort-indirection`: a runtime skill body (Claude or Codex) routes model/effort through the retired tier/cell indirection — "power-grade", the tier ladder ("the HIGH tier"), "cells" as a model noun, or a `model@effort` slug — instead of stating it directly. Frontmatter and fenced code blocks are skipped, and an inline `<!-- cog-skill-lint: allow-model-effort-indirection <reason> -->` on the preceding line records a deliberate exception. See [ADR-0036](../decisions/ADR-0036-explicit-model-and-effort-at-every-delegation.md) and "Explicit model and effort".
- `agent-launch-explicit-effort`: a fenced `cog codex-runner run-exec|run-resume` or `cog claude-runner run-exec` invocation in a runtime skill body or its `references/*.md` companions carries no literal `--effort` value from the launched runner’s own vocabulary (Codex: `minimal|low|medium|high|xhigh`; Claude: `low|medium|high|xhigh|max|none`), as an exact space-delimited token, judged quote-aware — quoted spans are argument data and an unquoted `#` ends the command — so a variable, a bare flag, an `--effort=<v>` spelling, a wrong-provider word, an extended token (`high5`), a commented-out flag, or effort words inside a quoted string do not count. Backslash-continued lines are joined into one logical command before matching; `--model` stays optional because omitting it accepts the harness default model. See [ADR-0036](../decisions/ADR-0036-explicit-model-and-effort-at-every-delegation.md) and "Explicit model and effort".
...
Work that writes to disk (implementation, saved plan artifacts, rewritten plans) must not run under Claude Code's native plan mode (`permission_mode = "plan"`, entered via `Shift+Tab` or `/plan`), which is read-only and blocks those writes. The gate lives on the executor- orchestrator layer: the caller a user launches gates once at entry, then delegates to gate-free plan/review workers. See [ADR-0017](../decisions/ADR-0017-skill-authoring-and-lint.md).
...
Every Claude `executor-*` skill carries a short Phase 0 pointer that keeps the STOP imperative in the always-loaded body and defers the full protocol to the shared source of truth, `skill-refs/orchestration/plan-mode-gate.md`:
...
Read-only Q&A relays (`ask`), inline same-context chainers (`executor-vetted`, `context-builder`), and verbatim transport relays (`gc`) do not cross a fresh-context boundary and carry no pointer; the human top-level operator orients the first skill directly and is exempt. The gate is a prose pointer to one skill-refs source of truth, not a stamped, lint-drift-checked stanza.
