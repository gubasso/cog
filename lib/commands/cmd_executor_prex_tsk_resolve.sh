...
: 'desc: Resolve a tsk issue for an executor-prex run.'

__cog_executor_prex_tsk_resolve_self_check='(.tsk_id|type=="string") and (.body_file|type=="string")'

__cog_executor_prex_tsk_resolve_usage() {
  cog::fn::ui_data "Usage: cog executor-prex-tsk-resolve --run-dir <dir> [--id <id>] [--json]"
}

__cog_executor_prex_tsk_resolve_resolve_id() {
...
__cog_executor_prex_tsk_resolve_build_json() {
  ...
  id="$(__cog_executor_prex_tsk_resolve_resolve_id "$run_dir" "$id")"
    ...
    case "$1" in
      ...
      --run-dir)
        [[ $# -ge 2 && -n ${2:-} ]] || cog::fn::error_raise "MissingArgument" \
          "missing run directory" "option: --run-dir" "" "run 'cog executor-prex-tsk-resolve --help'"
        run_dir="$2"
        shift 2
        ;;
      --id)
        [[ $# -ge 2 && -n ${2:-} ]] || cog::fn::error_raise "MissingArgument" \
          "missing tsk id" "option: --id" "" "run 'cog executor-prex-tsk-resolve --help'"
        id="$2"
        shift 2
        ;;
      --json)
      ...
      *)
        cog::fn::error_raise "InvalidInput" \
          "unknown executor-prex-tsk-resolve argument" "argument: $1" "" "run 'cog executor-prex-tsk-resolve --help'"
        ;;
    esac
  done
  [[ -n $run_dir ]] || cog::fn::error_raise "MissingArgument" \
    "missing run directory" "usage: cog executor-prex-tsk-resolve --run-dir <dir> [--id <id>] [--json]" "" \
    "run 'cog executor-prex-tsk-resolve --help'"
  json="$(__cog_executor_prex_tsk_resolve_build_json "$run_dir" "$id")"
  if [[ $mode == json || ${COG_UI_JSON:-false} == true ]]; then
    cog::fn::json_emit "$__cog_executor_prex_tsk_resolve_self_check" "$json"
  else
