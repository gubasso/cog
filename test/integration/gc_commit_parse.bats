...
@test "cog gc-commit-parse emits multi-repo lines and json" {
  local out="${BATS_TEST_TMPDIR}/gc.out"
  printf '%s\n' \
...
@test "cog gc-commit-parse fails if any repo failed" {
  local out="${BATS_TEST_TMPDIR}/gc.out"
  printf '%s\n' "COMMIT_OK abc1234 repo=/repo/a" "COMMIT_FAILED hook repo=/repo/b log=/tmp/log" >"$out"

  run --separate-stderr cog gc-commit-parse "$out" --json

  assert_failure
  [[ $stderr == *"gc commit failed"* ]]
}

@test "cog gc-commit-parse rejects failures and missing lines" {
  local out="${BATS_TEST_TMPDIR}/gc.out"
...
@test "cog gc-commit-parse --help dispatches" {
  run cog gc-commit-parse --help

  assert_success
  [[ $output == *"Parse gc commit"* ]]
}

@test "cog gc-commit-parse recognizes the COMMIT_OK empty marker" {
  local out="${BATS_TEST_TMPDIR}/gc.out"
  printf '%s\n' "COMMIT_OK empty" >"$out"
...
@test "cog gc-commit-parse reports empty:false for a real commit" {
  local out="${BATS_TEST_TMPDIR}/gc.out"
  printf '%s\n' "COMMIT_OK abc1234" >"$out"
